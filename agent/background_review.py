...
_SKILL_REVIEW_PROMPT = (
    ...
    "HARD GUARDS — these prevent wasted tool calls. Every one below has "
    "been observed to fire repeatedly in production when missing:\n"
    "  1. LOAD BEFORE PATCH. You MUST call skill_view(name=X) for skill X "
    "in the same review turn before you call skill_manage or write_file "
    "on X. The skill_manage tool will refuse patches for skills you have "
    "not loaded; those refusals cost a full LLM turn each. If you have "
    "not loaded X yet, load it first or skip X entirely this pass.\n"
    "  2. NO RETRY ON PROVENANCE-REFUSAL. If skill_manage returns a "
    "refusal for a skill that is not curator-managed (its usage record "
    "lacks the created_by: 'agent' marker, or has no record at all), the "
    "skill is off-limits to you. Do not propose the same "
    "patch again this pass, do not try a different action, do not fall "
    "back to write_file. Skip the skill, note the skip, move on. The "
    "user can run `hermes curator adopt <name>` in a foreground session "
    "if they want the curator to manage that skill.\n"
    "  3. ONE RETRY MAX PER SKILL. If a single skill_manage or write_file "
    "call fails for any reason, retry at most once, and only after "
    "verifying you have a fresh skill_view of the current SKILL.md "
    "contents in the same turn. If the retry still fails, give up on "
    "that skill for the rest of this review pass.\n"
    "  4. SKIP SKILLS YOU CANNOT PATCH. If a skill is bundled, hub-installed, "
    "pinned, in skills.external_dirs, or otherwise protected, do not "
    "attempt to read or modify its SKILL.md directly. The skill_manage "
    "tool's provenance check is the source of truth; trust its error.\n\n"
